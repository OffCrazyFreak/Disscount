...
| **A rounded tile gets plated twice**               | The first shortcut icons were rounded white tiles with a 52% glyph. Android reads a non-maskable icon as legacy artwork, shrinks it and drops it on a white plate of its own, so the glyph came out unreadably small and the tile's corners poked past the launcher's mask. Full bleed plus `purpose: "any maskable"` is the fix, and the same reasoning is why the app icon ships maskable at both 192 and 512. |
