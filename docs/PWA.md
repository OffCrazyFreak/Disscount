...
- [x] **Screen Wake Lock** while showing a loyalty-card barcode, in `hooks/use-wake-lock.ts`, held by the card view modal. There is no web API for brightness, so the code panel forces its own white background instead.
