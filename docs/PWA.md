...
Icons are generated from the happy-cart source (`public/brand/logo/cart/cart-white.svg`) by `scripts/generate-pwa-icons.mjs` (uses `sharp`), into `public/brand/icons` (plus the legacy `src/app/favicon.ico`). See `docs/BRAND.md` for the full asset map.

### Polarity

**Every home-screen tile is a white mark on a full-bleed brand green, never the reverse.** This is the single decision that made the icons read, and it took two failed attempts at growing the artwork to arrive at it.

A white plate does not read as an icon on a phone. Home screens are dense with saturated tiles, so a white circle reads as _empty space_ with something small floating in it, and it disappears entirely against a light wallpaper. The guidance is explicit: lean into Android's adaptive background layer with your brand's primary colour, and never leave a maskable icon transparent or white, because whatever the OS fills in "rarely looks intentional".

The practical consequence is that **size and polarity are not independent levers**. Enlarging a mark on a white plate is capped by the mask, and the cap arrives early: the scan glyph's corner brackets start clipping at about 0.74. Filling the tile has no cap at all, because the mark becomes the whole circle. Reach for polarity first and treat the ratios below as a refinement.

`cart-white.svg` already existed in the logo pack, so the generators select a variant rather than recolouring anything.

The splash screens are the deliberate exception: they stay a green cart on white, matching `background_color` so the launch does not flash.
...
**Icons.** Chrome accepts **PNG only** here and asks for 192x192. `scripts/generate-shortcut-icons.mjs` renders the same `lucide-react` component the nav uses through `renderToStaticMarkup`, then rasterizes it with `sharp` into `public/brand/shortcuts/<nav-item-id>.png`: a white glyph on a full-bleed brand-green tile, glyph at 66% (see [Icon sizing](#icon-sizing)). Rendering the component rather than a copied path is what keeps a shortcut icon from drifting away from its nav icon. `manifest.ts` derives each `src` from the nav item's `id`, so dropping a `comingSoon` flag wires a shortcut up by itself, and the only manual step left is adding its glyph to the generator and re-running it.

The tiles are declared `purpose: "any maskable"`. Full bleed plus `maskable` is the pair that matters: a tile carrying its own rounded corners is treated as legacy artwork, so Android shrinks it and drops it onto a second white plate, which both starves the glyph of size and leaves the tile's corners poking past the launcher's mask. A tile that ignores `purpose` still degrades gracefully, since Android's fallback plate simply frames the green square instead of blending into it.
...
| **Shortcut icons**       | `public/brand/shortcuts/*` from `scripts/generate-shortcut-icons.mjs`                                                                                                                                | one full-bleed 192 PNG per app shortcut, `any maskable`; Chrome takes PNG only, so the brand SVGs cannot be reused |
...
| `frontend/scripts/generate-shortcut-icons.mjs`                              | app-shortcut icon generator (white lucide glyph on a full-bleed green tile)                 |
...
| **A rounded tile gets plated twice**               | The first shortcut icons were rounded white tiles with a 52% glyph. Android reads a non-maskable icon as legacy artwork, shrinks it and drops it on a white plate of its own, so the glyph came out unreadably small and the tile's corners poked past the launcher's mask. Full bleed plus `purpose: "any maskable"` is the fix, and the same reasoning is why the app icon ships maskable at both 192 and 512. Growing the glyph on its own barely moved the needle; see [Polarity](#polarity) for what actually did. |
