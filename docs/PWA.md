...
| `/s/*` (shared shopping list pages)                 | **NetworkFirst**                           | cache `shared-list-pages`, 5s network timeout, 20 entries, 1 day. Must stay above `defaultCache`. See the note below |
...
| **Registration failures are unavoidable, and reach Sentry**       | `@serwist/next` injects the `navigator.serviceWorker.register()` call, so there is no call site of ours to catch its rejection on. Google's renderer stubs `register` to reject outright, and storage-restricted browsers fail too. Serwist then reads `registration.waiting` on a registration that was never assigned, so one failure arrives as two unrelated-looking issues (`Error: Rejected` plus a `waiting` TypeError). The app is fine either way, it just runs without offline caching. Filtered in `beforeSend` by `lib/sentry/ignore-service-worker-noise.ts`, which matches the message **and** a registration frame, because "Rejected" alone would swallow real errors. |
