...
| **pnpm's 24h supply-chain gate**                     | pnpm 11 enables `minimumReleaseAge` (1440 min) by default, so `pnpm install --frozen-lockfile` fails with `ERR_PNPM_MINIMUM_RELEASE_AGE_VIOLATION` when the lockfile holds a package published less than 24h ago. This blocks CI **and** the Docker build. Wait it out, or exempt one package via `minimumReleaseAgeExclude`. pnpm 11 reads this **only** from `pnpm-workspace.yaml`; the old `.npmrc` form is silently ignored. |
