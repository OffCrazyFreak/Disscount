...
**A plain `max-h` on the sheet loses to `ui/drawer.tsx`'s data-variant.** The shell asked for `max-h-[85dvh]`, but `drawer.tsx` carries `data-[vaul-drawer-direction=bottom]:max-h-[80vh]`. tailwind-merge keeps both, because the variant prefixes differ, and the compiled compound selector then outranks the plain class on specificity. Measured before the fix: a 735px viewport gave a 588px cap, exactly 80%, not the 625px the shell asked for. Worse, `vh` is the **large** viewport, so it ignores retractable browser chrome, which a sheet measured against the visible area cannot afford. It does **not** ignore the keyboard: `interactive-widget=resizes-content` shrinks the layout viewport itself, so every viewport unit shrinks with it, `vh` included. An earlier version of this line claimed otherwise and it is worth unlearning, because it sends you looking for a units bug when a keyboard buries something. The shell now states the cap with the same variant prefix, and it measures 624.75px.
...
**A list anchored to a trigger cannot be rescued from the keyboard by collision handling.** The pinned-places facet slid under the keyboard because its trigger sits in a scroll container that shrinks to a couple of hundred pixels when the keyboard opens, while `CommandList` asks for a flat 300px. Radix was positioning it correctly the whole time; there was simply no room. `MultiSelect` now picks by viewport: a popover from `md` up, and below `md` the list expands **in flow** under the trigger, where the browser's own scroll-focused-input-into-view does the work and there is no anchor left to get wrong. The popover path separately gained `collisionPadding` and a cap on `--radix-popover-content-available-height`, which `ui/select.tsx` and `ui/dropdown-menu.tsx` already had and Popover alone never did.
...
**cmdk's default filter is diacritic-blind in the wrong direction.** It lowercases and trims but leaves the caron alone, so `baska` never offered `Baška`. The multi-select now passes `scoreOption` from `utils/search-relevance.ts`, which normalizes both sides through `normalizeForSearch` and returns a score, so cmdk ranks as well as filters. That module holds the match-tier ladder lifted out of `product-relevance.ts`, which imports it rather than keeping a second copy. Where an option's value is a code rather than its label (chains), the label has to be named in cmdk's `keywords` or it is not searchable.
