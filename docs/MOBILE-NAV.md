...
**A Radix menu on a touchscreen opens on `pointerdown`, so a finger that was only starting to scroll springs it open** ([radix-ui/primitives#1912](https://github.com/radix-ui/primitives/issues/1912)). The fix is to swallow touch presses on the trigger, `preventDefault` so Radix skips its own handler, and resolve on release, abandoning any press that travelled past `HOLD_CANCEL_PX`; mouse and keyboard keep Radix's behaviour. Only the pointer that began the press may resolve it, or a second finger can toggle the menu out from under the first. This lived in `use-tap-to-open.ts`, deleted along with its one consumer, the shopping list's dropdown. **Three Radix menus still mount on touch unguarded**: `header-actions.tsx` renders `NotificationsDropdown` and `UserMenu` with no viewport or pointer gate, and `digital-cards/components/digital-card-item.tsx` mounts a third. None of them ever used the hook, so the bug above is live on all three. The note is here so whoever fixes them starts from the answer rather than rediscovering it.
