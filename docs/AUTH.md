...
- **That chain's own rule is `permitAll`, so the service-layer checks are the authentication boundary.** `UuidScopedRequestMatcher` keeps the chain to six routes, matched by method plus a UUID-shaped id: `/api/shopping-lists/me` and `/api/shopping-lists/items` stay authenticated because they are not UUIDs, not because they are listed as exceptions. See `docs/SHARING.md` §4.
