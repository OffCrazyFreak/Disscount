...
| --------------------------------------- | ----------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------- |
| `brand/logo/cart/cart-rgb.svg`          | The static cart, as stroked vector paths (the geometry)                                                           | `cart-source.mjs` -> splash, JSON-LD logo                            |
| `brand/logo/cart/cart-white.svg`        | The same geometry in white; generated by `generate-logos.mjs`, not hand-drawn                                     | `cart-source.mjs` -> every home-screen icon                          |
...
│   ├── icon.svg               # primary favicon (green plate + white cart)
...
| ---------- | -------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Primary    | `brand/icons/icon.svg`                 | Crisp at any size; used by Chrome, Edge, Firefox. Always a brand-green rounded plate with the white cart (kept consistent across light/dark browser themes) |
...
| ------------------------- | ---------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| `lib/cart-source.mjs`     | (helper) `renderCart(width, variant)`, `cartOnSquare(...)` | Strips the `<style>` block from the animated SVG to freeze the finished cart, then rasterizes it                     |
...
**Order matters** if you run them fresh: `generate-logos.mjs` first (it writes both `cart/cart-rgb.svg` and `cart/cart-white.svg`, which the other two read via `cart-source.mjs`), then the icon and splash scripts.
