...
The brand green `#2ec50d` lives in one constant so the scripts don't have to parse the SVG: `GREEN` in `scripts/lib/brand.mjs`, alongside `GREEN_RGB` for the pixel-level helpers. If you ever change the green, change it in the SVG sources **and** that module, then regenerate.
...
├── shortcuts/                 # PWA app-shortcut tiles (auto-wired), 2 per shortcut
...
frontend/public/qr-codes/      # QR codes pointing at disscount.me, for the cart and wordmark marks, in png / jpeg / svg
frontend/public/fonts/         # Saira Stencil SemiBold as woff2, so email clients can fetch the wordmark face
frontend/public/email/         # app screenshots and the author photo the marketing/emails/ templates link to
```
...
| Animated | `-animated.gif`  | Loops the draw-on animation, opening on the finished mark. `-rgb` has a **white matte**, `-white` is transparent |
| Animated | `-animated.webp` | Loops the draw-on animation, opening on the finished mark, always transparent, sRGB-tagged                       |
...
### What ships and what is hand-off

Not every file here is referenced by the app, and that is deliberate: this is a brand pack meant to be downloaded and handed to someone, not only imported.

- The **animated WebPs** (8 files, about 1.24 MiB) are referenced by nothing in the codebase. Their GIF twins are what the marketing emails load, because mail clients do not reliably render animated WebP. Keep both: the WebP is the better file to hand out for anything web-facing. Note the usual size expectation is inverted here, since `generate-logos.mjs` renders WebP at a larger width, so each one is heavier than its GIF twin rather than lighter.
- The **QR codes** exist for two marks in three formats each. Only `Disscount.me-qr-wordmark-rgb.png` is used, by both email templates. The other five are hand-off variants: the JPEGs for anywhere that rejects transparency, the SVGs for print. The SVGs are rasterised exports inside a vector wrapper rather than true vectors, which is why they are larger than the PNGs.

Regenerating the pack rewrites every one of these, so a logo script run is a large commit by nature.
...
**PWA icons** are declared in `src/app/manifest.ts`: `icon-192` and `icon-512` (`purpose: "any"`, cart at 86% to match `icon.svg`), plus `icon-maskable-192` and `icon-maskable-512` (`purpose: "maskable"`, cart at 70% so its diagonal clears Android's 80% safe-zone circle). The white plate is a deliberate brand call taken against the usual advice to fill Android's adaptive background with your primary colour; the green fill was built and compared before the white one was kept, so do not flip it without asking (see [Polarity](PWA.md#polarity)). Both maskable sizes ship because an `any` icon is not masked but shrunk onto Android's own white plate, and Chrome will reach for one if it wants a size the maskable set does not offer. The install surfaces (`install-banner.tsx` and `install-card.tsx`) reuse `icon-192.png`. Ratios and the reasoning behind them live in [`PWA.md`](PWA.md#icon-sizing).

**App-shortcut icons** (`brand/shortcuts/`, two 192 PNGs per shortcut) are the exception to "everything comes from the cart". They carry the same `lucide-react` glyph as the matching nav item, in white on a brand-green tile, because a shortcut has to read as its own destination rather than as the app. Each ships twice: `<id>.png` is full bleed for `purpose: "maskable"`, and `<id>-any.png` carries the brand corner radius for the surfaces that draw it unmasked. `scripts/generate-shortcut-icons.mjs` renders the icon component itself rather than a copied path, so the two can never drift. Chrome accepts PNG only here, which is why the SVGs cannot simply be linked. See [`PWA.md`](PWA.md#app-shortcuts).
...
All live in `frontend/scripts/` and are run manually with `node`. They share four helper modules, so the cart, its animation, the brand green and the sRGB tagging are each defined once.
...
| `lib/brand.mjs`               | (helper) `GREEN`, `GREEN_RGB`                             | The one definition of the brand green, so a palette change cannot land in one generator and be missed in another                   |
| `lib/srgb.mjs`                | (helper) `SRGB`, `srgbPng()`, `writeSrgbPng()`            | One sRGB tagging convention for every output; an untagged PNG is read as device-native and over-saturates the green                |
...
| Site OG / Twitter preview image                     | Automatic  | `src/app/opengraph-image.tsx`, `twitter-image.tsx`, rendered by `next/og`                |
...
| `frontend/src/app/favicon.ico`                                | Legacy favicon fallback (Next.js file convention)         |
| `frontend/src/app/opengraph-image.tsx`, `twitter-image.tsx`   | Site link-preview images, thin wrappers over the renderer |
| `frontend/src/app/og/render-og.tsx`                           | The shared OG renderer (brand plus CTA)                   |
...
| `frontend/scripts/lib/cart-frames.mjs`                        | Shared animation math                                     |
| `frontend/scripts/lib/brand.mjs`                              | The single brand-green constant                           |
| `frontend/scripts/lib/srgb.mjs`                               | Shared sRGB tagging for every generated raster            |
...
- `sharp` (`^0.35.3`): all rasterization, compositing, resizing, PNG/WebP/GIF encoding, and the sRGB profile tagging.
...
**Config / env / flags:** the brand system has **no env vars and no feature flags**. Absolute asset URLs in JSON-LD use the app origin via `appUrl()` (from `lib/env`), which returns `NEXT_PUBLIC_APP_URL` and fails closed in production so a missing var never leaks `localhost`; it falls back to localhost only in development. This is a general app variable, not brand-specific.
...
**Anything that shows one frame shows the first one.** Outlook's Word rendering engine, most inbox and chat previews, and PDF or print pipelines freeze an animation on frame 1. The draw-on starts from an empty canvas, so a naive export gave them a blank rectangle. `leadWithFinishedFrame()` in `generate-logos.mjs` moves the finished frame to the front and gives it the 1500 ms hold, so frame 1 is the complete mark and the loop lands back on it. The animation looks the same, it just starts one frame later in the cycle. If you add a new animated export, route it through `frames()` so it inherits this; do not build an Outlook-only still fallback, it is no longer needed.
