...
How to implement the findings the user selected. The goal is a clean, reviewable history: one commit per finding, each verified, on a branch that becomes one PR.
...
Group the selected findings into file-disjoint area batches (auth, forms/modals, products, backend, a11y, landing, copy, ...). Process batches sequentially in the single working tree so per-finding commit boundaries stay clean and there are no git races. Order areas most-critical first; within a batch, order importance then easiest-first.
...
Two findings that are literally the same edit share one commit and cite both IDs. Everything else is its own commit. This is an auto-commit workflow (the standing "never commit unless asked" rule is intentionally suspended for the fix phase, because the user asked for per-finding commits). No `Co-Authored-By` trailer. No em dashes.

## Verifying

Per batch (or per finding for risky ones), before committing:
...
After a batch, check whether it changed a behavior or invariant documented in `docs/*` (AUTH, STATE-PERSISTENCE, LANDING, BRAND, PWA, DEPLOYMENT). Update the affected gotchas / future-improvements surgically (do not rewrite the doc). Follow the doc conventions: no em dashes, one physical line per paragraph/bullet (no hard wrap). Format them with `pnpm exec prettier --write --ignore-path ../.prettierignore "../docs/<file>.md"` from `frontend/`. If a fix reveals a subsystem with no doc, ask before creating one (invoke the `document-subsystem` skill).
...
1. Final `tsc` check clean.
2. Push the branch (only after Checkpoint 2 branch confirmation).
