...
4. **Consolidate** only after every launched reviewer has finished (all `run FINISHED`, all Claude subagents returned). A slow or rate-limited tail is not a reason to go early; only a genuinely dead engine may be dropped, and then say what coverage is missing. One pass at the user's chosen model into the format(s) from `03-triage-doc-format.md`. If the user asks to see it early anyway, banner it in **bold** as NOT FINAL naming the outstanding reviewers, and when a straggler lands, ask with `AskUserQuestion` what to do with anything genuinely new.
