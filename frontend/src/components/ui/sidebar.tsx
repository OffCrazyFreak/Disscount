...
function Sidebar({
  ...
  // Mobile uses a drawer (not a plain sheet) so it can be swiped closed and
  // shows a grab handle. Width comes from the style prop, which beats the
  // drawer's direction-specific width classes.
  if (isMobile) {
    return (
      <Drawer direction={side} open={openMobile} onOpenChange={setOpenMobile}>
        <DrawerContent
          ...
          data-slot="sidebar"
          data-mobile="true"
          className="bg-sidebar text-sidebar-foreground max-w-8/10 p-0"
          style={
            {
...
const sidebarMenuButtonVariants = cva(
  // The active item is branded rather than shadcn's accent fill: bold primary
  // text, kept through hover by the extra specificity of data + hover.
  "peer/menu-button flex w-full items-center gap-2 overflow-hidden rounded-md p-2 text-left text-sm outline-hidden ring-sidebar-ring transition-[width,height,padding] hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 active:bg-sidebar-accent active:text-sidebar-accent-foreground disabled:pointer-events-none disabled:opacity-50 group-has-data-[sidebar=menu-action]/menu-item:pr-8 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-[active=true]:font-bold data-[active=true]:text-primary data-[active=true]:hover:text-primary data-[state=open]:hover:bg-sidebar-accent data-[state=open]:hover:text-sidebar-accent-foreground group-data-[collapsible=icon]:size-8! group-data-[collapsible=icon]:p-2! [&>span:last-child]:truncate [&>svg]:size-4 [&>svg]:shrink-0",
  {
    variants: {
      variant: {
...
function SidebarMenuBadge({
  ...
  return (
    <div
      ...
      className={cn(
        "text-sidebar-foreground pointer-events-none absolute right-1 flex h-5 min-w-5 items-center justify-center rounded-md px-1 text-[10px] sm:text-xs font-medium tabular-nums select-none sm:shadow-sm",
  ...
  return (
    <Comp
      ...
      className={cn(
        ...
        "data-[active=true]:font-bold data-[active=true]:text-primary data-[active=true]:hover:text-primary",
        size === "sm" && "text-xs",
        size === "md" && "text-sm",
        "group-data-[collapsible=icon]:hidden",
        className,
