...
// Next types purpose as a single value; the spec allows a space-separated list.
type ShortcutIcon = {
  src: string;
  sizes: string;
  type: string;
...
type Shortcut = Omit<
  NonNullable<MetadataRoute.Manifest["shortcuts"]>[number],
  "icons"
> & { icons?: ShortcutIcon[] };

// Neither of the last two keys is in Next's manifest type yet.
type WebAppManifest = Omit<MetadataRoute.Manifest, "shortcuts"> & {
  shortcuts?: Shortcut[];
...
// Maskable because the tiles are full bleed: without it Android shrinks them
// onto its own white plate and the glyph ends up too small to read.
